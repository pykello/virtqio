...
:::math
x^* = \argmin[x \in bb{R}^n] norm(Ax - b)_2
:::

where $A \in bb{R}^{m cross n}$ and $b \in bb{R}^m$.
...
:::math
Sigma^\dagger = \text{diag}(\dfrac{1}{sigma_1}, ..., \dfrac{1}{sigma_r}, 0, ..., 0)
:::
...
Given a matrix $A \in bb{R}^{m cross n}$ where $m >= n$ and $\text{rank}(A) = n$, we can factor it as $A = QR$, where:

- $Q \in bb{R}^{m cross m}$ is an orthogonal matrix (i.e., $Q^T Q = I_m$),
- $R \in bb{R}^{m cross n}$ is an upper trapezoidal matrix.
...
- $hat{Q} \in bb{R}^{m cross n}$ has orthonormal columns,
- $hat{R} \in bb{R}^{n cross n}$ is upper triangular.

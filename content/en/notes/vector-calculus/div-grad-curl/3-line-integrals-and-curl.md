...
:::math
int[C] f(x,y,z) \, ds = int[s_1..s_2] f(x(s),y(s),z(s)) \, ds
:::
...
:::math
W = int[C] v{f}(x,y,z) dot unit{t} \, ds
:::
...
:::math
unit{t} = v{i} dd(x, s) + v{j} dd(y, s) + v{k} dd(z, s)
:::

Then the work done can be expressed as:

:::math align
W &= int[C] v{f}(x,y,z) dot ( v{i} dd(x, s) + v{j} dd(y, s) + v{k} dd(z, s) ) ds
&= int[C] ( f_x dx + f_y dy + f_z dz )
:::
...
From earlier discussion, $W = int[C] (f_x dx + f_y dy + f_z dz)$.

So,

:::math
W = int[C] \frac{k q_0 q_1 (x dx + y dy + z dz)}{r^3}
:::

Since $r^2 = x^2 + y^2 + z^2$, then $dr = x dx + y dy + z dz$, and we have:

:::math
W = int[C] \frac{k q_0 q_1}{r^2} \, dr
:::
...
:::math
W = int[r_1..r_2] \frac{k q_0 q_1}{r^2} \, dr = k q_0 q_1 ( \frac{1}{r_1} - \frac{1}{r_2} )
:::
...
:::math align
int[C_B] v{F} dot unit{t} \, ds &\approx F_x (x, y - \frac{Delta y}{2}, z ) Delta x
int[C_T] v{F} dot unit{t} \, ds &\approx - F_x (x, y + \frac{Delta y}{2}, z ) Delta x
:::
...
:::math
lim[Delta S -> 0] \frac{1}{Delta S} int[C_T+C_B] ( v{F} dot unit{t} ) ds = -pd(F_x, y)
:::

Similarly:

:::math
lim[Delta S -> 0] \frac{1}{Delta S} int[C_L+C_R] ( v{F} dot unit{t} ) ds = pd(F_y, x)
:::

Combining these results, we have:

:::math
lim[Delta S -> 0] \frac{1}{Delta S} oint[C] v{F} dot unit{t} \, ds = pd(F_y, x) - pd(F_x, y)
:::

If our curve was an arbitrary shape, the result would still hold. But the result definitely
depends on the orientation.

If the curve were parallel to the $xz$-plane, we would get $pd(F_x, z) - pd(F_z, x)$, and if it were parallel to the $yz$-plane, we would get $pd(F_z, y) - pd(F_y, z)$.
...
- Curve parallel to $yz$, Normal vector $v{i}$: $pd(F_z, y) - pd(F_y, z)$
- Curve parallel to $xy$, Normal vector $v{j}$: $pd(F_x, z) - pd(F_z, x)$
- Curve parallel to $xz$, Normal vector $v{k}$: $pd(F_y, x) - pd(F_x, y)$
...
:::math
\text{curl} \, v{F} = detmat(v{i}, v{j}, v{k};
pd(\partial, x), pd(\partial, y), pd(\partial, z);
F_x, F_y, F_z)
:::
...
- If $\nabla cross v{F} = 0$, then $v{F}$ can be an electrostatic field.
- If $\nabla cross v{F} != 0$, then $v{F}$ cannot be an electrostatic field.
...
:::math
Delta I = v{J} dot unit{n} \, Delta S
:::

Then total current flowing through $S$ is:

:::math
iint[S] v{J} dot unit{n} \, dS
:::

Thus Ampere's circuital law can be written as:

:::math
oint[C] v{B} dot unit{t} \, ds = mu_0 iint[S] v{J} dot unit{n} \, dS
:::
...
:::math
iint[C] \nabla cross v{B} dot unit{n} \, dS = mu_0 iint[S] v{J} dot unit{n} \, dS
:::

Since this is true for any surface $S$ capping $C$, we can conclude that:

:::math
\nabla cross v{B} = mu_0 v{J}
:::
